                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        {!! Form::open(['route' => ['peliculas.update', $pelicula->idPelicula],'method' => 'PATCH']) !!}
                            ...
                            <div class="form-group">
                                    <label for="generos">Actores <span class="badge badge-info">{{count($act)}}</span></label>
                                    <div style="height:300px;overflow-y: scroll;">
                                      @foreach ($actores as $acts)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" {{\App\Pelicula::findActor($act,$acts->idActor) ? 'checked' : ''}} value="{{$acts->idActor}}" name="idActor[]">
                                            <label class="form-check-label">
                                                {{$acts->nombres}} {{$acts->apellidos}} 
                                            </label>
                                          </div>
                                      @endforeach
                                    </div>
